...
function main(): void {
  ...
  const data = JSON.parse(readFileSync(PHYSICS_FILE, "utf-8"));

  const settings: any[] = data.PhysicsSettings ?? [];
  ...
  for (let i = 0; i < settings.length; i++) {
    ...
    for (const key of ["Position", "Angle"]) {
      if (norm[key]) {
        const origMin: number = norm[key].Minimum;
        const origMax: number = norm[key].Maximum;
        const center = (origMin + origMax) / 2;
        const halfRange = (origMax - origMin) / 2;
        norm[key].Minimum = center - halfRange * RANGE_MULTIPLIER;
        norm[key].Maximum = center + halfRange * RANGE_MULTIPLIER;
      }
    }
